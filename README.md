...
- `crates/hsp-lsp` owns LSP route vocabulary and pure runtime primitives: chain parsing, command replacement, method preference maps, file URI encoding, language-id detection, and `Content-Length` JSON-RPC framing. Process spawning, broker routing, render memory, and edits build on this crate instead of copying Python `server.py` globals.
...
7. Port LSP routing/session management as runtime crates, not as DTO residue. The first Rust cut is `hsp-lsp`: it preserves Python `LSP_SERVERS`, legacy `LSP_COMMAND` fallback chain, `LSP_REPLACE`, `LSP_PREFER`, file URI, language-id, and LSP `Content-Length` framing rules before broker-owned process sessions land.
