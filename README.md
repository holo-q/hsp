...
4. Add daemon/client socket runtime on top of `hsp-protocol`, keeping socket serving and sync client transport outside `hsp-broker` dispatch. `hsp-client` and `hsp-daemon` now cover the synchronous client and `hsp-broker` binary skeleton.
5. Port LSP routing/session management as a runtime crate, not as DTO residue.
6. Preserve every behavior listed in the Python reference ledger or delete it by explicit design note.
